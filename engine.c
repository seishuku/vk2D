...
unsigned char GliderGun[36*9]=
{
	,
};

void Life(int width, int height, unsigned char *cells, unsigned char *next)
{
	int x, y;
	...
	for(y=0;y<height;y++)
	{
		for(x=0;x<width;x++)
		{
			unsigned char living=cells[y*width+x], result=0x00, count=0;
			...
			{
				if(cells[(y+1)*width+(x+1)])
					count++;
			}

			if(y!=height-1)
			{
				if(cells[(y+1)*width+x])
					count++;
			}
			...
			next[y*width+x]=result;
		}
	}

	memcpy(cells, next, renderWidth*renderHeight);
}

void Draw(void)
{
	Life(renderWidth, renderHeight, buffers[0], buffers[1]);
	...
	for(uint32_t y=0;y<renderHeight;y++)
	{
		...
		{
			if(buffers[0][y*renderWidth+x])
				DrawPixel(x, y, (float[]) { 1.0f, 1.0f, 1.0f });
		}
	}
...
bool Init(void)
{
	...
	for(uint32_t i=0;i<renderWidth*renderHeight;i++)
	{
		buffers[0][i]=0;
		buffers[1][i]=0;
	}
	...
	for(uint32_t i=0;i<36;i++)
	{
		for(uint32_t j=0;j<9;j++)
			buffers[0][(j+1)*renderWidth+(i+1)]=GliderGun[j*36+i];
	}

	for(uint32_t i=0;i<36;i++)
	{
		for(uint32_t j=0;j<9;j++)
			buffers[0][(j+1)*renderWidth+((renderWidth-36-1)+i)]=GliderGun[j*36+(36-i)-1];
	}

	for(uint32_t i=0;i<36;i++)
	{
		for(uint32_t j=0;j<9;j++)
			buffers[0][((renderHeight-9-1)+j)*renderWidth+(i+1)]=GliderGun[((9-j)-1)*36+i];
	}

	for(uint32_t i=0;i<36;i++)
	{
		for(uint32_t j=0;j<9;j++)
			buffers[0][((renderHeight-9-1)+j)*renderWidth+((renderWidth-36-1)+i)]=GliderGun[((9-j)-1)*36+((36-i)-1)];
	}
